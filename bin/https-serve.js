...
const https_conf = {
    key : fs.readFileSync(cwd+path.join("/ssl/private.key")),
    cert : fs.readFileSync(cwd+path.join("/ssl/certificate.crt")) ,
    ca : fs.readFileSync(cwd+path.join("/ssl/ca_bundle.crt")),
}
...
const startEndpoint = (endpoint, config, args, previous) => {
	...
	const server = https.createServer(https_conf,async (request, response) => {
		if (compress) {
